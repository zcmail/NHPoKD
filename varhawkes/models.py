...
class ModelHawkesVariational:
    ...
    def set_data(self, events):
        ...
        self.model.set_data(events)
        # Set various util attributes
        self.dim = len(events[0])
        self.n_jumps = len(events)*sum(map(len, events[0]))
        #参数个数加上distb的mean和viarance,即加2
        self.n_params = self.dim * (self.model.excitation.M * self.dim + 1) + 1
        self.n_var_params = 2 * self.n_params

    def _log_importance_weight(self, eps, alpha, beta, alpha2, beta2):
        """
        Compute the value of a single importance weight `log(w_i)`
        """
        # Reparametrize the variational parameters
        z, z2 = self.posterior.g(eps, alpha, beta, alpha2, beta2)
        ...
        W = z[self.dim:].reshape(self.dim, self.dim, self.model.excitation.M)
        distb = z2
        #print('z2 len is',len(z2))
        # Compute log-posterior
        logpost, logpost2 = self.posterior.logpdf(eps, alpha, beta, alpha2, beta2)
        # Evaluate the log-likelihood
        loglik = self.model.log_likelihood(mu, W, distb)
        # Compute the log-prior
        logprior , logprior2= self.prior.logprior(z, z2)
        return loglik + logprior + logprior2 - logpost - logpost2

    def _objective_l(self, eps_l, alpha, beta, alpha2, beta2):
        log_w_arr = torch.zeros(self.n_weights, dtype=torch.float64)
        for i in range(self.n_weights):
            eps = eps_l[i]
            # Compute the importance weights (and their gradients)
            log_w_arr[i] = self._log_importance_weight(eps, alpha, beta, alpha2, beta2)
        # Temper the weights
    ...
    def objective(self, x, seed=None):
        ...
        for l in range(self.n_samples):
            value += self._objective_l(eps_arr[l],  alpha, beta, alpha2, beta2)
